...
const updateData = async (oldData: Cell, ui: UIProvider) => {
    ...
    do {
        ...
        const updateMerkle = await promptBool(
        ...
        const updateSupply = await promptBool(
...
const upgradeAction = async (provider: NetworkProvider, ui: UIProvider) => {
    let upgradeCode = await promptBool(
...
export async function run(provider: NetworkProvider) {
    ...
    do {
        ...
        try {
            const verifyRes = await checkJettonMinter(
                { isBounceable: true, isTestOnly: false, address: minterAddress },
                minterCode,
                walletCode,
                provider,
                ui,
            ...
            jettonMinterContract = provider.open(JettonMinter.createFromAddress(minterAddress));
            adminAddress = await jettonMinterContract.getAdminAddress();
            ui.write('Ok, boss!');
            decimals = Number(await promptAmount('Please specify contract decimals:', 0, ui));
        }
    } while (retry);
