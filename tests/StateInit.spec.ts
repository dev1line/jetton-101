...
describe('State init tests', () => {
    beforeAll(async () => {
        ...
        jwallet_code_raw = jettonWalletCodeCell; //await compile('JettonWallet');
        minter_code = jettonMinterCodeCell; //await compile('JettonMinter');
    ...
    it('should mint max jetton walue', async () => {
        ...
        const res = await jettonMinter.sendMint(deployer.getSender(), deployer.address, maxValue, null, null, null);
