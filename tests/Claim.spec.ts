...
describe('Claim tests', () => {
    ...
    beforeAll(async () => {
        ...
        */
        ...
        getContractData = async (address: Address) => {
            ...
            if (!smc.account.account.storage.state.state.data) throw new Error('Data is not present');
    ...
    it('should claim and transfer', async () => {
        ...
        const res = await testJetton.sendTransfer(
            testReceiver.getSender(),
            toNano('1'),
            transferAmount,
            deployer.address,
    ...
    it('claim fee should be accounted for in transfer', async () => {
        ...
        let res = await testJetton.sendTransfer(
            testReceiver.getSender(),
            minimalTransfer,
            1n,
            deployer.address,
        ...
        res = await testJetton.sendTransfer(
            testReceiver.getSender(),
            toNano('0.12'),
            1n,
            deployer.address,
    ...
    it('claim fee should stay same regardless of jetton balance', async () => {
        const testJetton = await userWallet(testReceiver.address);
        const claimPayload = JettonWallet.claimPayload(receiverProof);
        await cMaster.sendMint(deployer.getSender(), testReceiver.address, 1n);
    ...
    it('claim fee should be dynamic based on dictionary lookup cost', async () => {
        ...
        */
        ...
        const transferMessage = JettonWallet.transferMessage(1n, deployer.address, testAddress, claimPayload, 1n);
    ...
    describe('Proofs', () => {
        it('should reject proof from different root', async () => {
            ...
            const res = await evilJetton.sendTransfer(
                evilDude.getSender(),
                toNano('1'),
                1n,
                testReceiver.address,
        ...
        */
            const fakeProof = beginCell().storeBuffer(airdropCell.hash(0)).storeUint(prunedPath.depth(0), 16).endCell();
        ...
        it('should reject proof for non-present address', async () => {
            ...
            let res = await evilJetton.sendTransfer(
                evilDude.getSender(),
                toNano('1'),
                1n,
                testReceiver.address,
            ...
            res = await evilJetton.sendTransfer(
                evilDude.getSender(),
                toNano('1'),
                1n,
                testReceiver.address,
