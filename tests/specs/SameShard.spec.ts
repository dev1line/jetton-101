...
describe('SameShard', () => {
    ...
    beforeAll(async () => {
        ...
        // minter_code = testMinterCodeCell; // await compile('JettonMinterTest');
        const wallet_code = jettonWalletCodeCell; // await compile('JettonWallet');
    ...
    it('should create wallet in closest shard on transfer', async () => {
        ...
        for (let i = 0; i < 100; i++) {
            try {
                const testAddress = new Address(0, await getSecureRandomBytes(32));
                const testJetton = await userWallet(testAddress);
                await deployerJetton.sendTransfer(
                    deployer.getSender(),
                    toNano('1'),
                    1n,
                    testAddress,
